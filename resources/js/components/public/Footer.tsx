            ...
            <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div className="lg:col-span-1">
                        <Link href="/" className="mb-6 inline-flex hover:opacity-90 transition-opacity">
                            <img src="/logo-t2r-branco.svg" alt="T2R Tecnologia" className="h-14 sm:h-16 w-auto" />
                        </Link>
                        <p className="mt-4 text-sm leading-relaxed text-white/50">
                            Soluções em GNSS, PPK, georreferenciamento e mapeamento com drones para
                    ...
                    {/* Acessos */}
                    <div>
                        <h3 className="mb-4 text-xs font-bold tracking-widest text-white/40 uppercase">Acessos</h3>
                        <ul className="space-y-2.5">
                            <FooterLink href="/produtos" label="Produtos" />
                            <FooterLink href="/metashape" label="Metashape" />
                            <FooterLink href="/sobre-nos" label="Sobre Nós" />
                            <FooterLink href="/blog" label="Blog" />
                            <FooterLink href="/fale-conosco" label="Fale Conosco" />
                        </ul>
                    </div>
...
function FooterLink({ href, label }: { href: string; label: string }) {
    return (
        <li><Link href={href} className="text-sm text-white/60 transition-colors hover:text-white">{label}</Link></li>
    );
}
...
function SocialLink({ href, label, d }: { href: string; label: string; d: string }) {
    return (
        <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label}
            className="flex h-8 w-8 items-center justify-center rounded-md bg-white/5 text-white/50 transition-colors hover:bg-white/10 hover:text-white">

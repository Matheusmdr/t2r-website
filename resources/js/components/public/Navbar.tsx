...
import type { NavItem } from '@/types/t2r';
            ...
            <div className={`fixed inset-0 z-40 bg-[#0a0a0a] transition-all duration-300 lg:hidden ${isMobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                <div className="flex h-full flex-col px-6 pt-20 pb-8">
                    ...
                    <div className="border-t border-white/10 pt-6 space-y-4">
                        <button
                            onClick={() => updateAppearance(appearance === 'dark' ? 'light' : 'dark')}
                            className="flex w-full items-center justify-center gap-2 rounded-full border border-white/10 px-6 py-3.5 text-base font-medium text-white hover:bg-white/5"
                        >
                            {appearance === 'dark' ? <><Sun className="h-5 w-5" /> Modo Claro</> : <><Moon className="h-5 w-5" /> Modo Escuro</>}
                        </button>
                        <a href="https://wa.me/5518997216319" target="_blank" rel="noopener noreferrer"
